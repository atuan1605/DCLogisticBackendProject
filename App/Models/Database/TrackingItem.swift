...
final class TrackingItem: Model, @unchecked Sendable {
    ...
    @OptionalField(key: "packed_at_vn_at")
    private(set) var packedAtVNAt: Date?
    
    @OptionalField(key: "pack_box_commited_at")
    private(set) var packBoxCommitedAt: Date?
    ...
    @Group(key: "broken_product")
    var brokenProduct: BrokenProduct

    @Children(for: \.$trackingItem)
    ...
    @OptionalParent(key: "pack_box_id")
    var packBox: PackBox?
    
    @Siblings(through: TrackingItemCustomer.self, from: \.$trackingItem, to: \.$customer)
    ...
    @OptionalField(key: "packing_video_id")
    var packingVideoFile: String?
    
    @Children(for: \.$trackingItem)
    var packingVideoQueues: [VideoDownloadingJob]
    
    @Children(for: \.$trackingItem)
    var trackingItemReferences: [TrackingItemReference]
    
    @Children(for: \.$trackingItem)
    var cameraDetails: [TrackingCameraDetail]
    
    @Siblings(through: BuyerTrackingItemLinkView.self, from: \.$trackingItem, to: \.$buyerTrackingItem)
    var buyerTrackingItems: [BuyerTrackingItem]
    
    @OptionalField(key: "hold_state")
    var holdState: HoldState?
    
    @OptionalField(key: "hold_state_at")
    var holdStateAt: Date?
    
    @Field(key: "all_access_check_tracking_with_images")
    var allAccessCheckTrackingWithImages: Bool
    ...
    init(
        ...
        self.packBoxCommitedAt = packBoxCommitedAt
        ...
        self.brokenProduct = .init(description: nil, customerFeedback: nil, flagAt: nil)
        self.$warehouse.id = warehouseID
        self.returnRequestAt = returnRequestAt
        self.allAccessCheckTrackingWithImages = true
    }
}
...
extension TrackingItem {
    ...
    var holdRequestContent: String? {
        if let buyerTrackingItems = self.$buyerTrackingItems.value, !buyerTrackingItems.isEmpty {
            let packingRequests = buyerTrackingItems.filter { !$0.packingRequest.isEmpty }.map { $0.packingRequest }
            if !packingRequests.isEmpty {
                return "[YCĐB] " + packingRequests.joined(separator: ", ")
            }
        }
        return nil
    }
	...
	func hasAllRequiredInformation(on db: Database) async throws -> Bool {
		let hasAgentCode = self.agentCode != nil
        let customers = try await self.$customers.get(on: db)
        let hasCustomerCode = customers.count > 0
		
		let products = try await self.$products.get(on: db)
		let hasProduct = products.count > 0
		
		let productHasInfo = products.contains(where: \.hasAllRequiredInformation)
		
		let isAlternativeRefFilled = !self.isWalmartTracking || self.alternativeRef?.isEmpty == false
		
		return hasAgentCode
			&& hasCustomerCode
			&& hasProduct
			&& productHasInfo
			&& isAlternativeRefFilled
	}
    ...
    func moveToStatus(to newStatus: Status, database: Database) async throws -> DeprecatedTrackingItemStatusUpdateJob.Payload? {
        ...
        guard self.returnRequestAt == nil else {
            if self.holdState == nil {
                self.holdStateAt = Date()
...
extension TrackingItem {
    ...
    func requireCameraDetail() -> TrackingCameraDetail? {
        if let cameraDetails = self.$cameraDetails.value, cameraDetails.count > 0 {
            return cameraDetails.first
        }
        return nil
    }
}
